...
namespace NAM {
HorizontalPriority* HorizontalPriority::instance = nullptr;
std::mutex HorizontalPriority::mutex;
HorizontalPriority* HorizontalPriority::getInstance() {
	if (instance == nullptr) {
		mutex.lock();
		if (instance == nullptr) {
			instance = new HorizontalPriority();
		}
		mutex.unlock();
	}
	return instance;
...
void HorizontalPriority::horizontalPriority(const cv::Mat& img, cv::Mat& dst, cv::Mat& seg, std::wstring& res)
{
	horizontalPriority(&img, dst, seg, res);
}
...
RNAMC::Location HorizontalPriority::findSameBlock(const cv::Mat* img, const cv::Mat* markMatrix, const cv::Mat* r, const int& x1, const int& y1, const double& margin, const double& ratio)
{
	int x2(x1), y2(y1);
